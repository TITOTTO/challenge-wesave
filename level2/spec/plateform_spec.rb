...
describe Plateform do
  let(:cars) { [Car.new(id: 1, price_per_day: 200, price_per_km: 10)] }
  ...
  describe "#rental_price" do
    it 'return rental price' do
      expect(plateform.rental_price(rentals.first)).to eq 1880
    end
  end

  describe "#reduction" do
    it 'return reduct rental price' do
      expect(plateform.price_after_discount(cars.first.price_per_day, rentals.first.duration)).to eq 880
    end
  end
end
